...
const Sidebar = () => {
  ...
  // Remove unwanted top-level sidebar sections
  const removeLabels = [
    "Layout",
    "Inventory",
    "Stock",
  ...
  const filteredSidebar = SidebarData?.filter(
    (item) => !removeLabels.includes(item?.label)
  );

  // Additionally remove unwanted items inside top-level 'Main' submenu
  const innerRemove = ["Super Admin", "Layouts", "Layout"];
  const cleanedSidebar = filteredSidebar?.map((item) => {
    if (item?.label === "Main" && Array.isArray(item.submenuItems)) {
      return {
        ...item,
        submenuItems: item.submenuItems.filter(
          (si) => !innerRemove.includes(si.label)
        )
      };
    }
    return item;
       ...
       <div data-simplebar="">
          <div className="sidebar-inner ">
            <div id="sidebar-menu" className="sidebar-menu">
              <ul>
                {cleanedSidebar?.map((mainLabel, index) =>
